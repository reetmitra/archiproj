import type { Metadata } from "next";
import { getJoinUs, getSiteSettings } from "@/lib/content";
import { PageIntro } from "@/components/primitives";

export const metadata: Metadata = {
  title: "Join us",
};

export default function JoinPage() {
  const join = getJoinUs();
  const site = getSiteSettings();
  const openings = join.openings.filter((o) => o.open);

  return (
    <>
      <PageIntro eyebrow="Join us" title={join.heading} lede={join.intro} />

      <div className="mx-auto max-w-4xl px-5 sm:px-8 pb-8">
        <section aria-labelledby="openings-heading">
          <h2
            id="openings-heading"
            className="font-mono text-sm uppercase tracking-[0.2em] text-stone pb-4 border-b-[3px] border-ink"
          >
            Current openings
          </h2>
          {openings.length > 0 ? (
            <div className="mt-8 space-y-6">
              {openings.map((opening) => (
                <article
                  key={opening.title}
                  className="border border-line rounded-sm p-6 sm:p-8"
                >
                  <div className="flex flex-wrap items-center gap-3">
                    <h3 className="font-display font-bold text-2xl tracking-tight">
                      {opening.title}
                    </h3>
                    <span className="bg-accent text-ink font-mono text-xs uppercase tracking-widest px-2 py-1 rounded-sm">
                      Open
                    </span>
                  </div>
                  <p className="mt-3 text-stone leading-relaxed">
                    {opening.description}
                  </p>
                </article>
              ))}
            </div>
          ) : (
            <p className="mt-8 text-stone leading-relaxed">
              No funded openings right now — but we always read thoughtful
              emails from prospective students, and positions open every year.
            </p>
          )}
        </section>

        <section aria-labelledby="apply-heading" className="mt-16">
          <h2
            id="apply-heading"
            className="font-mono text-sm uppercase tracking-[0.2em] text-stone pb-4 border-b-[3px] border-ink"
          >
            How to apply
          </h2>
          <p className="mt-8 text-lg leading-relaxed max-w-2xl">
            {join.howToApply}
          </p>
          <a
            href={`mailto:${site.email}`}
            className="mt-8 inline-block bg-ink text-paper font-display font-bold text-lg px-8 py-4 rounded-sm hover:bg-ink/85 transition-colors"
          >
            Write to us
          </a>
          <p className="mt-4 font-mono text-sm text-stone">{site.email}</p>
        </section>
      </div>
    </>
  );
}
